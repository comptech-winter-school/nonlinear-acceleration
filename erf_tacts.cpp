...
int main(int argc, char *argv[])
{
	...
	TCALL(Erf);					//замер тактов функции
	TCALL(Erf14);
	TCALL(erf);
	FILE * fout;
	...
	double end_point = 6;
	double interval = end_point - start_point;
	int N = 100000;
	double step = interval / N;
	ifstream inf("erf_sol.txt");  //Чтения файлов со значениям erf(x) из sollya
	if(!inf){
		cerr << "Not reading";
		exit(1);
	}
	...
	fout = fopen("err_erf14.txt", "w");
	if(fout != NULL){
		for(int i = 0; i < N; i++){
			fprintf(fout, "%.20lf %.20lf \n", a[i], abs(Erf14(a[i]) - a[i+1]));
			i+=1;
		}
	}
	fclose(fout);
